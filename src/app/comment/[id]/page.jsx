...
export default function Page() {
  ...
  useEffect(() => {
    ...
    // Reply
    getReplyCount(params.id).then((count) => {
      ...
      <div className={`__container ${styles.page__container}`} data-width={`medium`}>
              ...
              <section data-name={comment.name} className={`flex flex-column align-items-start justify-content-between`}>
                ...
                <main className={`${styles.post__main} w-100 flex flex-column grid--gap-050`}>
                  ...
                  <div onClick={(e) => e.stopPropagation()} className={`${styles.post__actions} flex flex-row align-items-center justify-content-start`}>
                    <Like id={comment.commentId} likeCount={comment.likeCount} hasLiked={post.hasLiked} />
        ...
        {mounted && isConnected && (
          <div className={`${styles.reply} flex align-items-center gap-025`} onClick={() => setShowCommentModal({data:post, type: `post`})}>
            ...
            <p>Reply to {address.slice(0, 4)}…${address.slice(38)}</p>
          </div>
        )}
      </div>
    ...
    <div className={`${styles.commentModal} animate fade`} onClick={() => setShowCommentModal()}>
      <div className={`${styles.commentModal__container}`} onClick={(e) => e.stopPropagation()}>
        ...
        <footer className={`${styles.commentModal__footer}  flex flex-column align-items-start`}>
          ...
          <textarea autoFocus defaultValue={commentContent} onInput={(e) => setCommentContent(e.target.value)} 
