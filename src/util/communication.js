...
export async function getRepliesByCommentId(commentId, startIndex, count) {
  const { web3, contract } = initPostCommentContract()

  try {
    const result = await contract.methods.getRepliesByCommentId(commentId, startIndex, count).call()
    return result
  } catch (error) {
    console.error('Error fetching contract data with Web3.js:', error)
    return { error }
...
export async function getComment(commentId) {
  ...
  try {
    const result = await contract.methods.getComment(commentId).call()
    return result
  } catch (error) {
    console.error('Error fetching contract data with Web3.js:', error)
    return { error }
